...
def clean_extracted_text(text):
    """
    Cleans the extracted text by processing each line.
    If a line has more than 12 elements when split by spaces, it truncates the line to keep only the first 12 elements.
    """
...
def create_data_tables(text):
    """
    Splits the cleaned text into data tables based on GPA grouping and constructs a Pandas DataFrame.

    Args:
        text (str): The cleaned text containing the data.

    Returns:
        pandas.DataFrame: A DataFrame where each row represents a GPA group and columns
                          represent the number of students in each category.
    """
    lines = text.splitlines()
    data = []
    for i, line in enumerate(lines):
